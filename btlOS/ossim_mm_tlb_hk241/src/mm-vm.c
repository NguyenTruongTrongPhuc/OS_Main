...
int enlist_vm_freerg_list(struct mm_struct *mm, struct vm_rg_struct *rg_elmt)
{
	...
	while (rg_node_walk != NULL)
	{
		...
		flag_can_merge = flag_merge_up | flag_merge_down;
...
int find_victim_page(struct mm_struct *mm, int *retpgn)
{
    ...
    if (pg == NULL) return 0; // Add a check for pg being NULL

    int flag_found = 0;

    while (pg != NULL) 
    {
        if (PAGING_PAGE_PRESENT(mm->pgd[pg->pgn])) // Check if the page is present
        {
            flag_found = 1;
            break;
        }
        
        pg = pg->pg_next; // Move to the next page
    }
